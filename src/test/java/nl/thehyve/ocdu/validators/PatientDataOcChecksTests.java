...
public class PatientDataOcChecksTests {
    ...
    public void testGetErrors() {
        ...
        List<ValidationErrorMessage> messageList =
                errorList.stream().filter(validationErrorMessage ->
                        validationErrorMessage.getMessageType() == MessageType.ERROR).collect(Collectors.toList());
    ...
    public void testDuplicateSubjectLabelInData() {
        ...
        assertThat(messageList.get(0).getMessage(), containsString("Duplicate subject ID found in data"));
        Collection<String> offendingValues = messageList.get(0).getOffendingValues();
    ...
    public void testDuplicatePersonIDInData() {
        ...
        List<ValidationErrorMessage> messageList =
                errorList.stream().filter(validationErrorMessage ->
                        validationErrorMessage.getMessageType() == MessageType.ERROR).collect(Collectors.toList());
        assertThat(messageList.get(0).getMessage(), containsString("Duplicate person ID present in subject data"));
        Collection<String> offendingValues = messageList.get(0).getOffendingValues();
    ...
    public void testDuplicateSubjectID() {
        ...
        List<ValidationErrorMessage> messageList =
                errorList.stream().filter(validationErrorMessage ->
                        validationErrorMessage.getMessageType() == MessageType.ERROR).collect(Collectors.toList());
        assertThat(messageList.get(0).getMessage(), containsString("One or more subjects are absent in the data file"));
    }
    ...
    public void testAutoGeneratedSubjectIDPatientCheck() throws Exception {
        ...
        assertThat(messageList.get(0).getMessage(), containsString("Study is configured to use auto-generated subject-ID's, subjects will not be created from subjects file."));
    }
    ...
    @Test
    public void testPersonIDPresentWarning() {

        PersonIdPresentWarning check = new PersonIdPresentWarning();
        Subject subject = new Subject();
        subject.setSsid("1234");
        subject.setPersonId("Pietje Petersen");

        ValidationErrorMessage error = check.getCorrespondingError(0, subject, metadata, testSubjectWithEventsTypeList, presentInData, subjectIDInSubjectDataList, personIDInSubjectDataList);
        assertThat(error.getMessage(), containsString("The subject registration file contains person ids"));
